...
class NMMLParser
{
   ...
   private function isTemplate(element:Access, section:String):Bool {
      if(isValidElement(element, section)) {
		 ...
		 else if (element.name == "assets"
         && element.has.type && element.att.type == "template")
			return true;
      }
      return false;
   }
      ...
      for(element in xml.elements) 
      {
         ...
         if (isValid) 
         {
            switch(element.name) 
            {
               ...
               case "assets":
				  if (!(element.has.type && element.att.type == "template") && !project.skipAssets)
               ...
               case "ios":
                  if (project.target == Platform.IOS || project.target == Platform.IOSVIEW ) 
                  {
                     ...
                     if (element.has.resolve("enable-bitcode")) 
                        project.iosConfig.enableBitcode = (substitute(element.att.resolve("enable-bitcode")) == "true");
                  }
               default:
